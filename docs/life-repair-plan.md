...
当前 `/life` 已具备完整闭环：开场动画、天赋选择、婴幼期自动叙事、事件选择、死亡、结算、成就、人生回顾。
...
当前婴幼期结束后直接跳到 6 岁，但不会立即触发 6 岁锚点事件，导致 `a_primary` 基本被跳过。
...
1. `rollD6()` 改为更符合百分制的初始化，例如 `rollInitialAttribute(): 35~55`。
