...
本轮新增 `deathRecord.attribute` 与模拟脚本 `deathAttributes` 统计，确认随机/激进策略的属性死亡主要来自：
...
```text
a_kid_epidemic：在家硬扛由必死改为严重肺炎后遗症。
a_young_fraud：买机票出发由必死改为逃脱后的严重损失。
a_young_lost：醉酒下海由必死改为溺水重创。
p_kid_ice：踩薄冰由必死改为落水重创。
p_kid_roof：爬屋顶由必死改为坠落骨折。
p_meme_gacha：抽卡沉船的财富损失下调，避免梗事件直接导致财富死亡。
```

200 局模拟对比：

```text
random 平均死亡年龄：16.55 -> 25.06
random 中位死亡年龄：13.5 -> 23
risky 平均死亡年龄：16.84 -> 25.44
risky 中位死亡年龄：11.5 -> 22
```

最新死亡属性统计：

```text
random：physique 111，wealth 73
risky：physique 122，wealth 53
safe：主要不是属性死亡，能稳定进入晚年
```
...
```text
早期误选导致的立即死亡已经下降。
随机/激进策略仍然高风险，但死亡主要来自连续低体质/低家境和明确危险青年事件。
safe 策略已进入可玩区间，不再过早终局。
```
...
本轮已在 `src/engine/events.ts` 增加普通事件软上限：

```text
每个年龄点最多触发 1 个普通参数化事件。
锚点事件不计入上限。
triggerEventId 强制事件不计入上限。
特殊篇章事件不计入上限。
```

同时在 `src/engine/reducer.ts` 中限制属性满值终局：

```text
60 岁前不会触发属性满值终局。
60 岁后属性达到 100 才进入对应属性终局。
```
...
```text
safe 中位终局年龄：19 -> 68
safe 平均事件数：56.99 -> 64.80
safe 平均梗事件：12.86 -> 7.15
random 平均死亡年龄：8.37 -> 16.55
risky 平均死亡年龄：8.95 -> 16.84
```

当前判断：

```text
安全策略已经能稳定进入晚年，并有一部分寿终正寝。
随机/激进策略仍高死亡，但死亡主要来自明确危险选择和低属性累积，属于可继续观察的高风险路线。
梗事件密度下降，但 safe 长寿局仍平均触发约 7 次，后续可按体验再调 meme 权重。
```

下一轮若继续平衡，优先项不再是总事件密度，而是：

```text
1. 降低早期危险锚点的即死集中度。
2. 微调 meme 事件权重或触发年龄，避免长寿安全局过多刷梗。
3. 检查随机策略下 attribute death 的具体属性，决定是否调低早期负收益或死亡阈值。
```
