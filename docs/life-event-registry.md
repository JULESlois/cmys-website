...
属性数值平衡应在事件数据中手动完成，不在结算层自动追加机会成本。主线互动选择如果存在纯正面收益，需要直接修改对应事件的 `attributes`，让选择在文本与数值上同时体现代价、取舍或风险。当前主线互动选择已完成一轮显式代价补充；事件仍继续使用原有百分制缩放表，以保持旧事件在 0~100 尺度下的影响力。特殊篇章仍默认不强制修改六维属性，优先通过 `chapterFlags`、关系变化、篇章进入/退出和结局状态表达代价。
